...
class LivesportScrapeError(RuntimeError):
    pass


class LivesportScraperProvider:
    ...
    def __init__(
        ...
        self.referer = str(referer).strip().rstrip("/") + "/"
    ...
    def _http_get_text_via_curl(self, url: str, headers: dict[str, str]) -> str:
        ...
        try:
            ...
            raise LivesportScrapeError("curl not found in PATH") from exc

        if proc.returncode != 0:
            raise LivesportScrapeError(f"curl failed ({proc.returncode}) for {url}: {proc.stderr.strip()}")
        out = proc.stdout
        if not out:
            raise LivesportScrapeError(f"Empty response from {url}")
        return out
    ...
    def _http_get_text(self, url: str, headers: dict[str, str]) -> str:
        ...
        raise LivesportScrapeError(f"HTTP fetch failed for {url}: {last_err}") from last_err
    ...
    def _http_get_json(self, url: str, headers: dict[str, str]) -> dict[str, Any]:
        ...
        try:
            ...
            raise LivesportScrapeError(f"Invalid JSON from {url}") from exc
        if not isinstance(payload, dict):
            raise LivesportScrapeError(f"Unexpected payload type from {url}: {type(payload).__name__}")
        return payload
    ...
    def _http_get_json_with_fallback(self, urls: list[str], headers: dict[str, str]) -> dict[str, Any]:
        ...
        for url in urls:
            try:
                ...
                last_err = exc
                continue
        raise LivesportScrapeError(f"All odds endpoints failed ({len(urls)} urls): {last_err}") from last_err
    ...
    def _fetch_feed_events(self) -> list[dict[str, Any]]:
        ...
        def flush_event() -> None:
            ...
            if len(event_id) >= 6 and home and away and isinstance(start_ts, int) and start_ts > 0:
                events.append(current_event)
            current_event = None

        mojibake_delim = chr(195) + chr(183)
        for token in tokens:
    ...
    def _fetch_h2h_for_bookmaker(self, event_id: str, bookmaker_id: int) -> tuple[float, float, float] | None:
        headers = {
            "Accept": "application/json",
            "User-Agent": "Mozilla/5.0",
            "Origin": self.referer.rstrip("/"),
    ...
    def load_events(self) -> list[Event]:
        ...
        if not feed_rows:
            self.last_diagnostics = {
                "source": f"{self.source_name}_scraper",
        ...
        for row in used_rows:
            ...
            events.append(
                Event(
                    event_id=f"{self.source_name}:{event_id}",
                    sport="football",
                    league=str(row.get("league") or "Football"),
                    home=str(row["home"]),
                    away=str(row["away"]),
...
def build_soccer24_provider(
    max_events: int = 40,
    geo_ip_code: str = "GB",
    geo_ip_subdivision_code: str = "GBENG",
    max_bookmakers_per_event: int = 8,
) -> LivesportScraperProvider:
    return LivesportScraperProvider(
        ...
        geo_ip_subdivision_code=geo_ip_subdivision_code,
        max_bookmakers_per_event=max_bookmakers_per_event,
    )


def build_livesport_provider(
    max_events: int = 40,
    geo_ip_code: str = "GB",
    geo_ip_subdivision_code: str = "GBENG",
    max_bookmakers_per_event: int = 8,
